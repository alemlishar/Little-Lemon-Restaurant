...
export function fetchAPI(date) {
  ...
  return JSON.parse(sessionStorage.getItem(requested)).map(
    (str) => new Date(str)
  )
}
...
export function submitAPI(formData) {
  const date = floorDate(formData.bookingDate)
  const available = fetchAPI(formData.bookingDate)

  console.log("api fetched timelist by submitted date:" + available)

  const index = available.findIndex(
    (x) => x.getTime() === formData.bookingDate.getTime()
  )
...
export function gethours(data) {
  let listOfHours = []
  if (data.length === 0) {
    listOfHours.push("No available time")
    console.log("reached here")
    return listOfHours
  }
  listOfHours = data.map((date, index) => {
    return date.getUTCHours() + ":00"
  })
  return listOfHours.sort()
}
